                ...
                <ul>
                    {state && state.map((todo, index) => (
                        <li key={todo.id} className="flex justify-between border-2 border-slate-300 my-2 p-2">
                            ...
                            <button onClick={() => {handleEditTodo(todo, index)}} style={{ textDecoration: todo.completed ? 'line-through' : '' }}>{todo.text}</button>
                            {/*<span onClick={() => { handleDeleteClick(index) }}>delete</span>*/}
